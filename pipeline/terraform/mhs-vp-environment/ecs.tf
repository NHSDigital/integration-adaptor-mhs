...
# Cloudwatch log group for MHS fake spine service to log to
resource "aws_cloudwatch_log_group" "mhs_fake_spine_log_group" {
  name = "/ecs/${var.environment_id}-mhs-fake-spine"
  tags = {
    Name = "${var.environment_id}-mhs-fake-spine-log-group"
    EnvironmentId = var.environment_id
  }
}
...
# Create an ECS task definition for the MHS fake spine container image.
resource "aws_ecs_task_definition" "mhs_fake_spine_task" {
  family = "${var.environment_id}-mhs-fake-spine"
  container_definitions = jsonencode(
  [
    {
      name = "mhs-fake-spine"
      ...
      environment = [
        ...
        },
        {
          ...
          value = var.fake_spine_outbound_delay_ms
        },
        {
          name = "FAKE_SPINE_INTBOUND_DELAY_MS"
          value = var.fake_spine_inbound_delay_ms
        },
        {
          name = "FAKE_SPINE_OUTBOUND_SSL_ENABLED"
          value = var.fake_spine_outbound_ssl_enabled
        },
        {
        ...
        },
        {
          ...
          value = var.fake_spine_inbound_proxy_port
        },
        {
          name = "FAKE_SPINE_PROXY_VALIDATE_CERT"
          value = var.fake_spine_proxy_validate_cert
        },
        {
      ...
      logConfiguration = {
        logDriver = "awslogs"
        options = {
          awslogs-group = aws_cloudwatch_log_group.mhs_fake_spine_log_group.name
          awslogs-region = var.region
          awslogs-stream-prefix = var.build_id
          awslogs-datetime-format = "\\[%Y-%m-%dT%H:%M:%S\\.%fZ\\]"
        }
  ...
  tags = {
    Name = "${var.environment_id}-mhs-fake-spine-task"
    EnvironmentId = var.environment_id
  }
  task_role_arn = var.task_role_arn
  execution_role_arn = var.execution_role_arn
...
# VPC's subnets. Each container is register with the fake spine service's LB's target group.
resource "aws_ecs_service" "mhs_fake_spine_service" {
  name = "${var.environment_id}-mhs-fake-spine"
  cluster = aws_ecs_cluster.mhs_cluster.id
  deployment_maximum_percent = 200
  deployment_minimum_healthy_percent = 100
  desired_count = var.mhs_fake_spine_service_minimum_instance_count
  launch_type = "FARGATE"
  scheduling_strategy = "REPLICA"
  task_definition = aws_ecs_task_definition.mhs_fake_spine_task.arn
  ...
  load_balancer {
    # In the MHS fake spine task definition, we define only 1 container, and for that container, we expose only 1 port
    # That is why in these 2 lines below we do "[0]" to reference that one container and port definition.
    container_name = jsondecode(aws_ecs_task_definition.mhs_fake_spine_task.container_definitions)[0].name
    container_port = jsondecode(aws_ecs_task_definition.mhs_fake_spine_task.container_definitions)[0].portMappings[0].hostPort
...
# The autoscaling target that configures autoscaling for the MHS route ECS service.
resource "aws_appautoscaling_target" "mhs_fake_spine_autoscaling_target" {
  max_capacity = var.mhs_fake_spine_service_maximum_instance_count
  min_capacity = var.mhs_fake_spine_service_minimum_instance_count
  resource_id = "service/${aws_ecs_cluster.mhs_cluster.name}/${aws_ecs_service.mhs_fake_spine_service.name}"
  role_arn = var.task_scaling_role_arn
  scalable_dimension = "ecs:service:DesiredCount"
  service_namespace = "ecs"
}

# An autoscaling policy for the MHS fake spine ECS service that scales services so that each instance handles the desired
# number of requests per minute.
resource "aws_appautoscaling_policy" "mhs_fake_spine_autoscaling_policy" {
  name = "${var.environment_id}-mhs-fake-spine-autoscaling-policy"
  policy_type = "TargetTrackingScaling"
  resource_id = aws_appautoscaling_target.mhs_fake_spine_autoscaling_target.resource_id
  scalable_dimension = aws_appautoscaling_target.mhs_fake_spine_autoscaling_target.scalable_dimension
  service_namespace = aws_appautoscaling_target.mhs_fake_spine_autoscaling_target.service_namespace

  target_tracking_scaling_policy_configuration {
    target_value = var.mhs_fake_spine_service_target_request_count
    predefined_metric_specification {
      predefined_metric_type = "ALBRequestCountPerTarget"
