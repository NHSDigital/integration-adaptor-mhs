...
##############
# MHS fake spine load balancer
##############

# Application load balancer for MHS fake spine
resource "aws_lb" "fake_spine_alb" {
  ...
  access_logs {
    bucket = aws_s3_bucket.mhs_access_logs_bucket.bucket
    prefix = "mhs_fake_spine-${var.build_id}"
    enabled = true
  }
...
# Target group for the application load balancer for MHS fake spine
# The MHS fake spine ECS service registers it's tasks here.
resource "aws_lb_target_group" "fake_spine_alb_target_group" {
  ...
  tags = {
    Name = "${var.environment_id}-mhs-fake-spine-alb-target-group"
    EnvironmentId = var.environment_id
  }
}

# Terraform output variable of the MHS fake spine load balancer's target group ARN
output "fake_spine_lb_target_group_arn" {
  value = aws_lb_target_group.fake_spine_alb_target_group.arn
  description = "The ARN of the MHS fake spine service load balancers's target group."
}

# Listener for MHS fake spine load balancer that forwards requests to the correct target group
