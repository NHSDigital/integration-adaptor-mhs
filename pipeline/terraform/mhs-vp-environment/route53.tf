...
# Route53 DNS record that is a domain name pointing to the MHS fake spine load balancer
resource "aws_route53_record" "mhs_fake_spine_load_balancer_record" {
  zone_id = aws_route53_zone.mhs_hosted_zone.zone_id
  name = "mhs-fake-spine.${aws_route53_zone.mhs_hosted_zone.name}"
  type = "A"
...
# Terraform output variable of the MHS fake spine load balancer Route53 domain name
output "fake_spine_lb_domain_name" {
  value = aws_route53_record.mhs_fake_spine_load_balancer_record.name
  description = "The DNS name of the Route53 record pointing to the MHS fake spine service's load balancer."
}
