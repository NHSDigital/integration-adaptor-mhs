...
class PikaQueueAdaptor(comms.queue_adaptor.QueueAdaptor):
    ...
    def __construct_message(self, message: dict, properties: Dict[str, Any] = None) -> proton.Message:
        ...
        return aio_pika.Message(message_id=message_id, content_type='application/json', body=json.dumps(message).encode(),
                                correlation_id=properties.get('correlation-id'), expiration=self.ttl_in_seconds
                                )

    async def __try_sending_to_all_in_sequence(self, message: aio_pika.Message) -> None:
        """
        ...
        for url in self.urls:
            try:
                ...
                connection = await aio_pika.connect("amqps://<login>:<password>@<aws-broker-url>/")
    ...
    async def __send_with_retries(self, message: aio_pika.Message) -> None:
        """
...
class PikaMessagingHandler(proton.handlers.MessagingHandler):
